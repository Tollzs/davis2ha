                        ...
                        if type(j[k]) is list:
                            ...
                            for l in data:
                                if l == "ts":
                                    ...
                                    self.set_state("sensor.weatherlink_eadtime", state = readtime, attributes = {"friendly_name": "Weatherlink time"})
                                if l == "temp_out":
                                    self.set_state("sensor.weatherlink_temp_out", state = data[l], attributes = {"friendly_name": "Outside Temperature", "unit_of_measurement": "°F"})
                                elif l == "hum_out":
                                    self.set_state("sensor.weatherlink_hum_out", state = data[l], attributes = {"friendly_name": "Outside Humidity", "unit_of_measurement": "%"})
                                elif l == "wind_speed":
                                    self.set_state("sensor.weatherlink_wind_speed", state = data[l], attributes = {"friendly_name": "Wind Speed", "unit_of_measurement": "MPH"})
                                elif l == "wind_speed_10_min_avg":
                                    self.set_state("sensor.weatherlink_wind_speed_10_min_avg", state = data[l], attributes = {"friendly_name": "Wind Speed 10 min avg", "unit_of_measurement": "MPH"})
                                elif l == "bar":
                                    self.set_state("sensor.weatherlink_bar", state = data[l], attributes = {"friendly_name": "Barometric Pressure", "unit_of_measurement": "inHg"})
                                elif l == "wind_dir":
                                    self.set_state("sensor.weatherlink_wind_dir", state = data[l], attributes = {"friendly_name": "Wind Direction", "unit_of_measurement": "°"})
                                elif l == "bar_trend":
                                    self.set_state("sensor.weatherlink_bar_trend", state = data[l], attributes = {"friendly_name": "Barometric Trend", "unit_of_measurement": "in/hr"})
                                elif l == "rain_rate_clicks":
                                    if isinstance(data[l], int):
                                        rainr = data[l] / 100
                                    else:
                                        rainr = 0
                                    self.set_state("sensor.weatherlink_rain_rate_clicks", state = rainr, attributes = {"friendly_name": "Rain Rate", "unit_of_measurement": "in/hr"})
                                elif l == "uv":
                                    self.set_state("sensor.weatherlink_uv", state = data[l], attributes = {"friendly_name": "UV Index"})
                                elif l == "solar_rad":
                                    self.set_state("sensor.weatherlink_solar_rad", state = data[l], attributes = {"friendly_name": "Solar Radiation", "unit_of_measurement": "W/m²"})
                                elif l == "rain_storm_clicks":
                                    if isinstance(data[l], int):
                                        rains = data[l] / 100
                                    else:
                                        rains = 0
                                    self.set_state("sensor.weatherlink_rain_storm_clicks", state = rains, attributes = {"friendly_name": "Rain Storm", "unit_of_measurement": "in"})
                                    ...
                                    if isinstance(data[l], int):
                                        ...
                                        raind = 0
                                    self.set_state("sensor.weatherlink_rain_day_clicks", state = raind, attributes = {"friendly_name": "Current Day Rain", "unit_of_measurement": "in"})
                                elif l == "rain_month_clicks":
                                    if isinstance(data[l], int):
                                        rainm = data[l] / 100
                                    else:
                                        rainm = 0
                                    self.set_state("sensor.weatherlink_rain_month_clicks", state = rainm, attributes = {"friendly_name": "Current Month Rain", "unit_of_measurement": "in"})
                                elif l == "rain_year_clicks":
                                    if isinstance(data[l], int):
                                        rainy = data[l] / 100
                                    else:
                                        rainy = 0
                                    self.set_state("sensor.weatherlink_rain_year_clicks", state = rainy, attributes = {"friendly_name": "Current Year Rain", "unit_of_measurement": "in"})
                                elif l == "et_day":
                                    self.set_state("sensor.weatherlink_et_day", state = data[l], attributes = {"friendly_name": "Current Day Evapotranspiration", "unit_of_measurement": "in"})
                                elif l == "et_month":
                                    self.set_state("sensor.weatherlink_et_month", state = data[l], attributes = {"friendly_name": "Current Month Evapotranspiration", "unit_of_measurement": "in"})
                                elif l == "et_year":
                                    self.set_state("sensor.weatherlink_et_year", state = data[l], attributes = {"friendly_name": "Current Year Evapotranspiration", "unit_of_measurement": "in"})
                                elif l == "dew_point":
                                    self.set_state("sensor.weatherlink_dew_point", state = data[l], attributes = {"friendly_name": "Dew Point", "unit_of_measurement": "°F"})
                                elif l == "heat_index":
                                    self.set_state("sensor.weatherlink_heat_index", state = data[l], attributes = {"friendly_name": "Heat Index", "unit_of_measurement": "°F"})
                                elif l == "wind_chill":
                                    self.set_state("sensor.weatherlink_wind_chill", state = data[l], attributes = {"friendly_name": "Wind Chill", "unit_of_measurement": "°F"})
                                    ...
                                    self.set_state("sensor.weatherlink_wet_leaf", state = data[l], attributes = {"Wet Leaf": "Forecast Description", "unit_of_measurement": "°F"})
